...
function Filterbar({
  ...
  const generateComplaintFetchLink = () => {
    ...
    // Add the user's districts as areaD parameter
    if (currentUser.Districts && currentUser.Districts.length > 0) {
      url.searchParams.set("areaD", currentUser.Districts.join(','));
    }
  
    // Add the user's sub-districts as areaSubD parameter
    if (currentUser.SubDistricts && currentUser.SubDistricts.length > 0) {
      url.searchParams.set("areaSubD", currentUser.SubDistricts.join(','));
    }
